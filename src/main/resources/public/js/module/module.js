layui.use(['table', 'treetable'], function () {
    ...
    table.on('toolbar(munu-table)', function (data) {
        ...
        if (data.event == "expand") {
            // 全部展开
            treeTable.expandAll("#munu-table"); //-->通过id选择器选中表格,调用全部打开方法

        } else if (data.event == "fold") {
            // 全部折叠
            treeTable.foldAll("#munu-table"); //-->通过id选择器选中表格,调用全部折叠方法
    ...
    table.on('tool(munu-table)',function (data) {
        ...
        if (data.event == "add") {
            ...
            openAddModule(data.data.grade+1, data.data.id);

        } else if (data.event == "edit") {
            // 修改资源
            openUpdateModule(data.data.id);
     ...
     */
    function openAddModule(grade, parentId) {
